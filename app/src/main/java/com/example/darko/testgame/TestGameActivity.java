...
/**
 * Created by darko on 6/30/2016.
 */
public class TestGameActivity extends SimpleBaseGameActivity {
    private static final String TAG = TestGameActivity.class.getSimpleName();
...
// 6 - Add touch handlers
    ...
    private void checkForCollisionsWithTowers(Ring ring) {
        ...
        stack.add(ring);
        checkIfFinished(stack);
        ring.setmStack(stack);
        ring.setmTower(tower);
    }

    private void checkIfFinished(Stack stack) {
        if (stack.size() == mStack3.size() && stack.size() == 3) {
            finish();
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(TestGameActivity.this, "Goog game, Bravoo!", Toast.LENGTH_SHORT).show();
                }
            });
    ...
    public EngineOptions onCreateEngineOptions() {
        ...
        CroppedResolutionPolicy crp = new CroppedResolutionPolicy(CAMERA_WIDTH, CAMERA_HEIGHT);
